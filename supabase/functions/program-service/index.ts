...
- Provide one distinct workout session for each of the client's ${profile.daysPerWeek} training days per week, with every session fully detailed. Label each session on its own line with its training focus, exactly as Workout One (Push), then Workout Two (Lower Body), then Workout Three (Pull), and so on following the rotation rule below, then list that session's exercises on the lines beneath its label. Do not name days of the week.
- Training split (mandatory): Never program a full body day. Each session focuses on exactly one of Push (chest, shoulders, triceps), Pull (back, biceps, rear delts), or Lower Body (quads, hamstrings, glutes, calves). Push and Pull are both upper body sessions. Rotate so upper body and lower body alternate every training day: each upper body day (Push or Pull) is immediately followed by a Lower Body day, and each Lower Body day is followed by an upper body day. Never place two upper body days (Push and Pull) back to back, and never repeat the same focus on two consecutive training days. Alternate the upper body days between Push and Pull. For example, 3 days: Push, Lower Body, Pull. 4 days: Push, Lower Body, Pull, Lower Body. 5 days: Push, Lower Body, Pull, Lower Body, Push. 6 days: Push, Lower Body, Pull, Lower Body, Push, Lower Body. Include direct core and ab work on as many of the sessions as makes sense for the client, on any focus day, placed after the main compound lifts.
...
- Portion the per meal macros so that choosing one Breakfast option, one Lunch option, one Dinner option, and the Snack adds up to roughly the daily Calorie, Protein, Carb, and Fat Targets. Keep the four options within each meal close to the same calories and macros as one another, so whichever option the client picks keeps the day on target.
...
- Each meal option must list its exact ingredients with portion sizes (for example: 4 ounces grilled chicken, 1 cup brown rice, 1 cup steamed broccoli, 1 tablespoon olive oil), then a busy day substitution, then end with that meal's nutrition in this exact format: Macros: NNN calories, NNg protein, NNg carbs, NNg fat. Put the word Macros and the four values at the very end of that meal's single line, with realistic numbers for the portions listed. Do this for every Breakfast, Lunch, and Dinner option and for the Snack, Pre Workout, and Post Workout.
